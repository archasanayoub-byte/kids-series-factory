...
اعمل commit + push، بس **الرفع لليوتيوب ما بيصير فوراً**. السيستم صار "طابور نشر" (release queue):
- الـ workflow بيشتغل **3 مرات باليوم** (06:00 / 10:00 / 14:00 UTC = 09:00 / 13:00 / 17:00 بتوقيت إسرائيل بالصيف)
- كل مرة بيرفع **حلقة وحدة بس** (الأقدم بالطابور اللي لسا ما انرفعت)
- يعني حط كذا حلقة مرة وحدة بمجلد `to_upload`، وهي رح تنشر تلقائياً وحدة كل 4 ساعات، مو كلها دفعة وحدة

لو بدك تنشر كل الحلقات المعلّقة دفعة وحدة (تجاوز الطابور)، شغّل الـ workflow يدوياً من تبويب Actions مع تفعيل خيار `PUBLISH_ALL` (أو غيّر متغير البيئة بالـ workflow مؤقتاً لـ `PUBLISH_ALL: "1"`).

بعد كل رفع: الفيديو والصورة المصغرة بيتحذفوا من المجلد (ما نخزن فيديوهات كبيرة بالـ git)،
والميتاداتا بتنتقل لـ `videos/uploaded/`، والـ manifest بيتحدث - كل هذا commit تلقائي من الـ bot.

⚠️ إسرائيل بتطلع من التوقيت الصيفي (DST) نهاية أكتوبر تقريباً — لو بدك تحافظ على نفس أوقات النشر المحلية بالشتاء، لازم تعدّل الـ cron بملف `.github/workflows/youtube_publish.yml` لـ `"0 5,9,13 * * *"` (ساعة أبكر بالـ UTC).
